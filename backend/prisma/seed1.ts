...
const FOLDERS = [
  "b2c/asics_gel_kayano_32/silver",
  "b2c/asics_gel_kayano_32/blue",
  "b2c/asics_gel-pulse_17/green",
  "b2c/asics_gel-pulse_17/black",
  "b2c/asics_gel_resolution_8/mint",
] as const;
...
> = {
  asics_gel_kayano_32: {
    ...
    description:
      "ASICS GEL-KAYANO 32 is built for comfortable, stable road mileage. Supportive platform, smooth transitions, and plush feel for daily training and long runs.",
    weightGrams: 280,
    lengthMm: 330,
    widthMm: 220,
    heightMm: 130,
  ...
  "asics_gel-pulse_17": {
    ...
    surface: ShoeSurface.ROAD,
    description:
      "ASICS GEL-PULSE 17 is a versatile daily trainer for road running and gym use, offering balanced cushioning and comfort for steady mileage.",
    weightGrams: 270,
    lengthMm: 330,
    widthMm: 220,
    heightMm: 130,
  },
  asics_gel_resolution_8: {
    priceCents: 14000,
    ratingAvg: "4.60",
    ratingCount: 52,
    surface: ShoeSurface.INDOOR,
    description:
      "ASICS GEL-RESOLUTION 8 is designed for tennis stability and durability, supporting lateral movement with a secure fit and solid court traction.",
    weightGrams: 320,
    lengthMm: 340,
    widthMm: 230,
    heightMm: 140,
...
async function prepareData(): Promise<PreparedProduct[]> {
  ...
  for (const [modelSlug, colorFolders] of byModel.entries()) {
    ...
    const categorySlug = modelSlug.includes("resolution") ? "tennis" : "running";

    const meta =
      PRODUCT_META[modelSlug] ?? {
        priceCents: 0,
        ratingAvg: "0.00",
        ratingCount: 0,
        surface: ShoeSurface.ROAD,
    ...
    for (const folder of colorFolders) {
      ...
      const colorHex = COLOR_HEX[colorName.toLowerCase()] ?? null;
      const stockQty = STOCK_BY_COLOR[colorName.toLowerCase()] ?? DEFAULT_STOCK_QTY;
      ...
      const images = assets
        .map((a) => {
          ...
          if (!view || !a.secure_url || !a.public_id) return null;
...
export async function seedBatch1(prisma: PrismaClient) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const prepared of preparedProducts) {
      const category =
        prepared.categorySlug === "tennis" ? catTennis : catRunning;
      ...
      for (const preparedColor of prepared.colors) {
        ...
        const incomingPublicIds = new Set(preparedColor.images.map((i) => i.publicId));
        const incomingSkus = new Set(preparedColor.skus.map((s) => s.sku));

        // Upsert images
        for (const image of preparedColor.images) {
          await tx.productColorImage.upsert({
            where: {
              publicId: image.publicId, // requires @unique on publicId
            },
            update: {
              colorId: color.id,
              url: image.url,
        ...
        for (const sku of preparedColor.skus) {
          await tx.productSku.upsert({
            where: { sku: sku.sku }, // requires sku @unique
            update: {
              colorId: color.id,
              barcode: sku.barcode,
              sizeEU: sku.sizeEU,
        ...
        console.log(
          `Seeded ${preparedColor.folder}: images=${preparedColor.images.length} skus=${preparedColor.skus.length} price=${prepared.meta.priceCents} rating=${prepared.meta.ratingAvg}(${prepared.meta.ratingCount})`
        );
      }
    }
  });
