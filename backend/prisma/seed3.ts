...
const FOLDERS = [
  "b2c/nike_wildhorse_10/platinum",
  "b2c/nike_vomero_18/blue",

  "b2c/nike_vomero_premium/black",
  "b2c/nike_vomero_premium/alabastar",

  "b2c/nike_free_metcon_6/light violet",
  "b2c/nike_free_metcon_6/white",
] as const;
...
> = {
  nike_wildhorse_10: {
    ...
    description:
      "NIKE WILDHORSE 10 is built for trail running with confident traction and protection underfoot. A secure fit and responsive cushioning help you stay comfortable on uneven terrain and longer off-road efforts.",
    weightGrams: 300,
    lengthMm: 335,
    widthMm: 230,
    heightMm: 140,
  ...
  nike_vomero_18: {
    ...
    description:
      "NIKE VOMERO 18 delivers plush comfort for daily road miles. Designed for smooth transitions and long-run cushioning, it offers a stable, comfortable ride for training and recovery runs.",
    weightGrams: 290,
    lengthMm: 335,
    widthMm: 225,
    heightMm: 135,
  ...
  nike_vomero_premium: {
    ...
    surface: ShoeSurface.ROAD,
    description:
      "NIKE VOMERO PREMIUM is a comfort-first road runner with a premium feel. Soft cushioning, supportive fit, and a smooth ride make it ideal for long easy runs and all-day wear.",
    weightGrams: 305,
    lengthMm: 340,
    widthMm: 230,
    heightMm: 140,
  ...
  nike_free_metcon_6: {
    ...
    description:
      "NIKE FREE METCON 6 blends flexibility and stability for training. A supportive base for lifting plus agile forefoot feel makes it a strong pick for gym sessions, circuits, and cross-training.",
    weightGrams: 320,
    lengthMm: 330,
    widthMm: 235,
    heightMm: 140,
...
async function acquireSeedLock(tx: Prisma.TransactionClient) {
  await tx.$executeRawUnsafe(`SELECT pg_advisory_xact_lock(987654322)`);
}
...
async function prepareData(): Promise<PreparedProduct[]> {
  ...
  for (const [modelSlug, colorFolders] of byModel.entries()) {
    ...
    const categorySlug = modelSlug.toLowerCase().includes("metcon")
      ? "training"
      : "running";
    ...
    for (const folder of colorFolders) {
      ...
      const stockQty =
        STOCK_BY_COLOR[colorName.toLowerCase()] ?? DEFAULT_STOCK_QTY;
...
export async function seedBatch3(prisma: PrismaClient) {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const prepared of preparedProducts) {
      const category =
        prepared.categorySlug === "training" ? catTraining : catRunning;
